...
function useTableData(props, { dataRelatedColsRef }) {
    const selectionColumnRef = vue_1.computed(() => {
    ...
    const treeMateRef = vue_1.computed(() => {
        ...
        return treemate_1.createTreeMate(props.data, {
            getKey: props.rowKey,
    ...
    const firstContentfulColIndexRef = vooks_1.useMemo(() => {
        const { columns } = props;
    ...
    const uncontrolledFilterStateRef = vue_1.ref({});
    const uncontrolledSortStateRef = vue_1.ref(null);
    const uncontrolledCurrentPageRef = vue_1.ref(1);
    const uncontrolledPageSizeRef = vue_1.ref(10);
    ...
    const controlledCurrentPageRef = vue_1.computed(() => {
        const { pagination } = props;
        if (pagination === false)
            return undefined;
        return pagination.page;
    });
    const controlledPageSizeRef = vue_1.computed(() => {
        const { pagination } = props;
        if (pagination === false)
            return undefined;
        return pagination.pageSize;
    });
    const mergedCurrentPageRef = vooks_1.useMergedState(controlledCurrentPageRef, uncontrolledCurrentPageRef);
    const mergedPageSizeRef = vooks_1.useMergedState(controlledPageSizeRef, uncontrolledPageSizeRef);
    ...
    const mergedSortStateRef = vue_1.computed(() => {
    ...
    const mergedFilterStateRef = vue_1.computed(() => {
        ...
        const activeFilters = Object.assign(utils_1.createShallowClonedObject(uncontrolledFilterStateRef.value), controlledFilterState);
        return activeFilters;
    });
    const filteredDataRef = vue_1.computed(() => {
    ...
    const sortedDataRef = vue_1.computed(() => {
        ...
        if (activeSorter) {
            ...
            const sorter = activeSorter.sorter === undefined || activeSorter.sorter === 'default'
                ...
                : activeSorter.sorter;
            return filteredData.sort((tmNode1, tmNode2) => utils_1.getFlagOfOrder(order) * sorter(tmNode1.rawNode, tmNode2.rawNode));
        }
        return filteredDataRef.value;
    });
    const paginatedDataRef = vue_1.computed(() => {
        if (props.remote)
    ...
    const rawPaginatedDataRef = vue_1.computed(() => {
    ...
    const mergedPaginationRef = vue_1.computed(() => {
    ...
    function doUpdatePage(page) {
        ...
        if (onUpdatePage)
            _utils_1.call(onUpdatePage, page);
        if (onPageChange)
            _utils_1.call(onPageChange, page);
    ...
    function doUpdateSorter(sortState) {
        ...
        if (onUpdateSorter)
            _utils_1.call(onUpdateSorter, sortState);
        if (onSorterChange)
            _utils_1.call(onSorterChange, sortState);
    ...
    function doUpdateFilters(filters, sourceColumn) {
        ...
        if (onUpdateFilters)
            _utils_1.call(onUpdateFilters, filters, sourceColumn);
        if (onFiltersChange)
            _utils_1.call(onFiltersChange, filters, sourceColumn);
    ...
    function filter(filters) {
        if (!filters) {
            ...
            doUpdateFilters(utils_1.createShallowClonedObject(filters));
        }
        else if (process.env.NODE_ENV !== 'production') {
            _utils_1.warn('data-table', '`filters` is not an object');
        }
    }
    return {
        treeMateRef,
