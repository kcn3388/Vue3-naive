...
const selectProps = Object.assign(Object.assign({}, _mixins_1.useTheme.props), { to: _utils_1.useAdjustedTo.propTo, bordered: {
        ...
        default: null
    }, value: [String, Number, Array], placeholder: String, multiple: {
        type: Boolean,
        default: false
    }, size: String, filterable: {
        type: Boolean,
        default: false
    }, disabled: {
        type: Boolean,
        ...
        default: (pattern, option) => {
            ...
            if (typeof option.label === 'string') {
                return utils_1.patternMatched(pattern, option.label);
            }
            else if (option.value !== undefined) {
                return utils_1.patternMatched(pattern, String(option.value));
            }
            return false;
        }
    }, placement: {
    ...
    onChange: {
        ...
        validator: () => {
            if (process.env.NODE_ENV !== 'production') {
                _utils_1.warn('select', '`on-change` is deprecated, please use `on-update:value` instead.');
            }
            return true;
        },
        default: undefined
        ...
        validator: () => {
            if (process.env.NODE_ENV !== 'production') {
                _utils_1.warn('select', '`items` is deprecated, please use `options` instead.');
            }
            return true;
        },
        default: undefined
...
exports.default = vue_1.defineComponent({
    ...
    props: selectProps,
    setup(props) {
        const { mergedClsPrefixRef, mergedBorderedRef, namespaceRef } = _mixins_1.useConfig(props);
        const themeRef = _mixins_1.useTheme('Select', 'Select', index_cssr_1.default, styles_1.selectLight, props, mergedClsPrefixRef);
        ...
        const treeMateRef = vue_1.computed(() => treemate_1.createTreeMate(filteredOptionsRef.value, utils_1.tmOptions));
        const valOptMapRef = vue_1.computed(() => utils_1.createValOptMap(props.options));
        ...
        const menuRef = vue_1.ref(null);
        const { localeRef } = _mixins_1.useLocale('Select');
        const localizedPlaceholderRef = vue_1.computed(() => {
            var _a;
            return (_a = props.placeholder) !== null && _a !== void 0 ? _a : localeRef.value.placeholder;
        });
        const compitableOptionsRef = vooks_1.useCompitable(props, [
            'items',
            'options'
        ]);
        const createdOptionsRef = vue_1.ref([]);
        const beingCreatedOptionsRef = vue_1.ref([]);
        const memoValOptMapRef = vue_1.ref(new Map());
        const wrappedFallbackOptionRef = vue_1.computed(() => {
        ...
        const localOptionsRef = vue_1.computed(() => {
        ...
        const filteredOptionsRef = vue_1.computed(() => {
            if (props.remote) {
                ...
                if (!pattern.length || !props.filterable) {
                    ...
                    const { filter } = props;
                    return utils_1.filterOptions(localOptions, filter, pattern);
                }
            }
        });
        ...
        function doSearch(value) {
            ...
            if (onSearch)
                _utils_1.call(onSearch, value);
        }
        function doScroll(e) {
            const { onScroll } = props;
            if (onScroll)
                _utils_1.call(onScroll, e);
        }
        ...
        function handleDeleteLastOption() {
        ...
        function handleMenuMousedown(e) {
            if (!seemly_1.happensIn(e, 'action') && props.multiple && props.filterable) {
                focusSelection();
            }
        }
        ...
        vue_1.watch(vue_1.toRef(props, 'options'), updateMemorizedOptions);
        vue_1.watch(filteredOptionsRef, () => {
            if (!mergedShowRef.value)
                return;
    ...
    render() {
        ...
        return (vue_1.h("div", { class: `${mergedClsPrefix}-select` },
            vue_1.h(vueuc_1.VBinder, null, {
                default: () => [
                    vue_1.h(vueuc_1.VTarget, null, {
                        default: () => (vue_1.h(_internal_1.NInternalSelection, { ref: "triggerRef", clsPrefix: mergedClsPrefix, showArrow: this.showArrow, maxTagCount: this.maxTagCount, bordered: this.mergedBordered, active: this.mergedShow, pattern: this.pattern, placeholder: this.localizedPlaceholder, selectedOption: this.selectedOption, selectedOptions: this.selectedOptions, multiple: this.multiple, filterable: this.filterable, remote: this.remote, clearable: this.clearable, disabled: this.disabled, size: this.mergedSize, theme: this.mergedTheme.peers.InternalSelection, themeOverrides: this.mergedTheme.peerOverrides.InternalSelection, loading: this.loading, autofocus: this.autofocus, focused: this.focused, onClick: this.handleTriggerClick, onDeleteLastOption: this.handleDeleteLastOption, onDeleteOption: this.handleToggleOption, onPatternInput: this.handlePatternInput, onClear: this.handleClear, onBlur: this.handleTriggerBlur, onFocus: this.handleTriggerFocus, onKeydown: this.handleKeyDown, onKeyup: this.handleKeyUp }))
                    }),
                    vue_1.h(vueuc_1.VFollower, { ref: "followerRef", show: this.mergedShow, to: this.adjustedTo, teleportDisabled: this.adjustedTo === _utils_1.useAdjustedTo.tdkey, containerClass: this.namespace, width: this.consistentMenuWidth ? 'target' : undefined, minWidth: "target", placement: "bottom-start" }, {
                        default: () => (vue_1.h(vue_1.Transition, { name: "fade-in-scale-up-transition", appear: this.isMounted, onLeave: this.handleMenuLeave }, {
                            default: () => (this.mergedShow ||
                                ...
                                vue_1.withDirectives(vue_1.h(_internal_1.NInternalSelectMenu, { ref: "menuRef", virtualScroll: this.consistentMenuWidth && this.virtualScroll, class: `${mergedClsPrefix}-select-menu`, clsPrefix: mergedClsPrefix, focusable: true, autoPending: true, theme: this.mergedTheme.peers.InternalSelectMenu, themeOverrides: this.mergedTheme.peerOverrides
                                        .InternalSelectMenu, treeMate: this.treeMate, multiple: this.multiple, size: "medium", value: this.mergedValue, style: this.cssVars, onMenuToggleOption: this.handleToggleOption, onScroll: this.handleMenuScroll, onFocus: this.handleMenuFocus, onBlur: this.handleMenuBlur, onKeyup: this.handleMenuKeyup, onKeydown: this.handleMenuKeydown, onTabOut: this.handleMenuTabOut, onMousedown: this.handleMenuMousedown, show: this.mergedShow }, $slots), this.displayDirective === 'show'
                                    ? [
